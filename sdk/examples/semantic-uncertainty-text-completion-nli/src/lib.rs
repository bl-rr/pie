//! Semantic Uncertainty Text Completion (NLI variant).
//!
//! Same as semantic-uncertainty-text-completion but uses a dedicated DeBERTa NLI
//! model for entailment classification instead of LLM-based entailment.
//! This dramatically speeds up semantic checking by offloading the O(n²) entailment
//! checks to a lightweight classifier model with batched inference.
...
async fn main(mut args: Args) -> Result<String> {
    ...
    let nli_queue = model.create_queue();
    ...
    // Compute semantic entropy using NLI model
    let semantic_start = Instant::now();
    let mut judge = NliEntailmentJudge::new(&nli_queue);
    let semantic_ids = get_semantic_ids(&responses, &mut judge, &prompt).await;
    ...
    let output = serde_json::json!({
        ...
        "config": {
            ...
            "nli_model": "deberta-v3-nli (lazy-loaded)",
        }
    });
