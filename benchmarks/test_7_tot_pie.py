...
from pie_eval_common import (
    add_auth_args,
    connect_client,
    ensure_inferlet_installed,
    random_math_prompt,
...
async def main(args):
    ...
    try:
        ...
        def _build_instance_args(_index: int):
            return [
                "--question",
                random_math_prompt(),
                "--num-branches",
                str(args.num_branches),
                "--max-tokens",
                str(args.max_tokens),
            ]
...
if __name__ == "__main__":
    ...
    wasm_args_group.add_argument(
        "--num-branches", type=int, default=2, help="Number of branches to explore."
    )
    wasm_args_group.add_argument(
        "--max-tokens", type=int, default=64, help="Maximum tokens per step."
    )
